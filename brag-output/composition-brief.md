...
- Duration: 23.7s target (hard bound: 15–25s)
...
- `verified 29   blocked 9   (of 47)`
- `NOT LAUNCHABLE.`
- `The blockers are legal, not code.`
...
- **Hook (0–2.5s):** The headline `Their scan arrives before they do.` revealed
  ...
  "introducing."
- **Outro / punchline:** `The blockers are legal, not code.` on white, MIR's
...
Use the storyboard in `brag-output/brag-plan.md` as the creative contract. Scene
summary:

1. **The promise** — 4.2s — eyebrow + `Their scan arrives before they do.` revealed
...
5. **The turn** — 4.5s — full-bleed dark terminal, all mono. `$ pnpm verify:gates`
   ...
   summary `verified 29   blocked 9   (of 47)`; a held half-beat of nothing; then
   `NOT LAUNCHABLE.` in `#b3261e`, holding 1.4s in silence.
6. **The close** — 2.5s — white, near-empty. `The blockers are legal, not code.`
...
- **Audio files:** copy the chosen music and any selected SFX into
  `brag-output/composition/assets/`. Paths in the composition HTML must be
...
Every on-screen claim is verified against the tree as of 2026-09-20. Do not add
any that are not:

- `verified 29   blocked 9   (of 47)` and `NOT LAUNCHABLE.` — exact live output of
