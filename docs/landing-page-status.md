...
| **L0** Fonts chosen; licences bought if commercial | ✅ | IBM Plex Sans Arabic (incl. Light 300) + IBM Plex Mono + Google Sans Flex, all OFL, self-hosted and subset. `app/fonts/README.md` records each file, its role and its bytes. No commercial licence needed; §3.2's optional 29LT upgrade was not taken. |
...
| **L2** Tokens, `Plate`, type scale, per-script scaling, grain, focus states | ✅ | `app/corridor.css`. §3.1 palette, §3.2 scale with `--script-scale` 1.08 and `--leading-body` 1.5 / `--leading-display` 1.25 under RTL, §3.3 plate with corner ticks, §3.5 motion tokens. The grain/scanline/bloom materials this row used to cite were retired by the 2026-09-10 re-theme — see the §2 deviation row. |
